...
Several additional exports are available from alkali for working with elements. The first is `applyProperties` which allows the same type of element properties (like `content`, `classes`) that constructors can take, along with variable values, to be applied to an existing element. It takes the form:
`applyProperties(targetElement, properties)`
